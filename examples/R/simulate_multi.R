...
model {
  ...
  net <-
    ...
    finact <- Simulate0(net,0.1)
    for(i in 1:N) {
      for(j in 1:M) {
        p.o1[j,i] <- exp(finact[5,j,i])/(exp(finact[5,j,i]) + exp(finact[6,j,i]))
      }
    }
}"


jres <- jags(data=list(), model.file = textConnection(modelstring), DIC = F, parameters.to.save = c("finact","p.o1"))
jres