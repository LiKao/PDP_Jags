...
model {
  ...
  m <- Net2Mat(net, c, -c, -f, tau(val[1],p), tau(val[2],p), tau(val[3],p), tau(val[4],p) )
}"


jres <- jags(data=list(), model.file = textConnection(modelstring), DIC = F, parameters.to.save = c("f","c","p","m"),)
jres
